...
with base as (
  select *
  from `carbon-watch-455617-j1.public.film_actor` --Your table path
  )
  ...
  , final as (
    select
        actor_id
        , film_id
        , last_update as actor_film_last_update
   FROM base
  )
...
schema = [
    ...
    bigquery.SchemaField('actor_film_last_update', 'DATETIME'),
    ]
